...
async function saveImport(rows, userId) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      // Проверка дубликата: пропускаем только если совпадает номер + компания + премия
      if (row.number) {
        const dupeFilter = {
          userId,
          number: { $regex: new RegExp('^' + escapeRegex(row.number) + '$', 'i') }
        };
        if (row.company) {
          dupeFilter.company = { $regex: new RegExp('^' + escapeRegex(row.company) + '$', 'i') };
        }
        if (row.premium !== null && row.premium !== undefined) {
          dupeFilter.premium = row.premium;
        }
