...
**Последняя задача:** 4.1 (Модель Task + API) — ГОТОВО
**Следующая задача:** 4.2 (Страница задач — фронтенд)
**Блокеры:** нет
**Тестирование:** API задач (4.1) — ожидает тестирования. Фаза 3 полностью протестирована.
...
_21.03.2026 — Задача 4.1 (Модель Task + API). Создан Task.js: схема с userId, clientId (опционально, ref Client), title (required, 2-500 символов), dueDate, priority (h/m/l, default m), done (default false), timestamps, индекс userId+done+dueDate. Создан taskService.js: getTasks (filter all/active/done, sort, pagination, populate clientId), getTaskById, createTask (проверка clientId если указан), updateTask (whitelist полей, проверка clientId, обнуление clientId), toggleTask (переключение done), deleteTask. Создан routes/tasks.js: GET /, POST /, PUT /:id, PATCH /:id/toggle, DELETE /:id. Обновлён app.js: заглушка tasks заменена на реальный роутер. Также: ClientPicker обновлён — при фокусе показывает всех клиентов (GET /clients?limit=200&sort=name), фильтрация на клиенте при вводе. Далее: 4.2 (страница задач фронтенд)._
